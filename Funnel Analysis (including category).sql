-- The result from this query was connected to Power BI to build a dashboard.
-- It is slightly different from data used in the excel visualization with the category included.
...
key_funnels AS (
  SELECT category, country, event_name, COUNT(*) as number_of_events
  FROM raw_events
  ...
  GROUP BY category,country, event_name
  ORDER BY category,country,COUNT(*) DESC
),
